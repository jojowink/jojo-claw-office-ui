#!/usr/bin/env python3
"""
JOJO办公室 - Agent 状态主动推送脚本

用法：
1. 填入下面的 JOIN_KEY（你从JOJO那里拿到的一次性 join key）
2. 填入 AGENT_NAME（你想要在办公室里显示的名字）
3. 运行：python office-agent-push.py
4. 脚本会自动先 join（首次运行），然后每 30s 向JOJO办公室推送一次你的当前状态
"""
...
OFFICE_URL = "https://office.hyacinth.im"  # JOJO办公室地址（一般不用改）
...
def do_join(local):
    ...
    if r.status_code in (200, 201):
        ...
        if data.get("ok"):
            ...
            print(f"✅ 已加入JOJO办公室，agentId={local['agentId']}")
            return True
    print(f"❌ 加入失败：{r.text}")
    return False
