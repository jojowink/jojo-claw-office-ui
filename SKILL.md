...
"搬新家 / 找中介"装修功能需要 Seedream API，但**基础看板不需要**，不装也能正常使用。
...
../skills/gemini-image-generate/.venv/bin/pip install google-genai pillow
```

安装完成后，后端会自动检测到生图环境，"搬新家 / 找中介"按钮即可使用。

### 4.2 配置 Seedream API Key

引导用户完成这两项配置：

1. `SEEDREAM_API_KEY`
2. `SEEDREAM_MODEL`（推荐：`seedream-5-lite`）

配置方式有两种：
- **侧边栏填写**：打开资产侧边栏 → 在生图配置区域直接输入 API Key 并保存
- **环境变量**：`export SEEDREAM_API_KEY="your-key"`
...
提醒主人：
- 生图功能需要自己的 API；
- 当前接入的是 **Volcengine Seedream API**；
...
推荐配置：
- `SEEDREAM_API_KEY`
- `SEEDREAM_MODEL=seedream-5-lite`
...
4. 保留并检查本地运行配置：
   - `ASSET_DRAWER_PASS`
   - `GEMINI_API_KEY` / `GEMINI_MODEL`（如需生图，注意新版推荐使用 `SEEDREAM_API_KEY`）
5. 如有自定义位置，确认：
...
3. 已支持 **接入自有生图 API** 持续更换背景（推荐 `seedream-5-lite`）。
