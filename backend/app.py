...
GEMINI_SCRIPT_CANDIDATES = [
    os.path.join(ROOT_DIR, "scripts", "gemini_image_generate.py"),
    os.path.join(WORKSPACE_DIR, "skills", "gemini-image-generate", "scripts", "gemini_image_generate.py"),
]
GEMINI_SCRIPT = next((p for p in GEMINI_SCRIPT_CANDIDATES if os.path.exists(p)), GEMINI_SCRIPT_CANDIDATES[0])
GEMINI_PYTHON_CANDIDATES = [
    os.path.join(WORKSPACE_DIR, "skills", "gemini-image-generate", ".venv", "bin", "python"),
    sys.executable,
]
GEMINI_PYTHON = next((p for p in GEMINI_PYTHON_CANDIDATES if os.path.exists(p)), sys.executable)
...
def _provider_model_candidates(user_model: str):
    ...
    return list(USER_MODEL_TO_PROVIDER_MODELS.get(normalized, USER_MODEL_TO_PROVIDER_MODELS["seedream-5-lite"]))
...
def _generate_rpg_background_to_webp(out_webp_path: str, width: int = 1280, height: int = 720, custom_prompt: str = "", speed_mode: str = "fast"):
    ...
    speed_mode:
      - fast: use seedream-5-lite + 1024x576 intermediate + downscaled reference (faster)
      - quality: use configured model (fallback seedream-5-lite) + full 1280x720 path
    """
    runtime_cfg = load_runtime_config()
    api_key = (runtime_cfg.get("seedream_api_key") or "").strip()
    if not api_key:
    ...
    configured_user_model = _normalize_user_model(runtime_cfg.get("seedream_model") or runtime_cfg.get("gemini_model") or "seedream-5-lite")
    if mode == "fast":
        preferred_user_model = "seedream-5-lite"
    ...
    allow_aspect_ratio = (preferred_user_model == "seedream-5-lite")
    ...
    # 模型多级回退（仅允许两类用户模型：seedream-5-lite / seedream-5）
...
def _bg_generate_worker(task_id: str, custom_prompt: str, speed_mode: str):
    ...
    try:
        ...
        if msg == "MISSING_API_KEY":
            ...
            error_result["msg"] = "Missing SEEDREAM_API_KEY"
...
def assets_generate_rpg_background():
    ...
    try:
        ...
        if not api_key:
            api_key = (runtime_cfg.get("gemini_api_key") or "").strip()
        if not api_key:
            return jsonify({"ok": False, "code": "MISSING_API_KEY", "msg": "Missing SEEDREAM_API_KEY"}), 400
        if not (os.path.exists(GEMINI_PYTHON) and os.path.exists(GEMINI_SCRIPT)):
...
def gemini_config_get():
    ...
    try:
        ...
        key = (cfg.get("seedream_api_key") or "").strip()
        if not key:
        ...
        return jsonify({
            ...
            "gemini_model": _normalize_user_model(cfg.get("seedream_model") or cfg.get("gemini_model") or "seedream-5-lite"),
        })
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)}), 500
...
def gemini_config_set():
    ...
    try:
        ...
        model = _normalize_user_model((data.get("model") or "").strip() or "seedream-5-lite")
        payload = {"seedream_model": model}
        if api_key:
            payload["seedream_api_key"] = api_key
        save_runtime_config(payload)
        return jsonify({"ok": True, "msg": "Seedream 配置已保存"})
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)}), 500
